...
- If the pub cache is empty and `flutter pub get` fails while formatting generated localizations, run `flutter clean` first, then rerun `flutter pub get`.
  **Why:** a stale `.dart_tool/package_config.json` can point at missing analysis-option packages; a clean rebuild lets Pub restore the locked packages correctly.
  **How to apply:** use `flutter clean && flutter pub get`, then regenerate localizations and run the normal build checks.
</content>
